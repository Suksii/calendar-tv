...
export default function ShowCatalog({
  ...
  async function handleAdd(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!newTitle.trim()) {
      toast.error("Polje ne smije biti prazno!");
      return;
    }
    setLoading(true);
    try {
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Greška pri brisanju emisije.",
      );
    }
  }
        ...
        {isAdmin && (
          <form onSubmit={handleAdd} className="flex gap-2">
            <input
              ...
              placeholder="Unesite naziv emisije"
              ...
              {shows.map((show) => (
                ...
                >
                  ...
                  {isAdmin && (
                    <button
                      onClick={() =>
                        setConfirm({ id: show.id, title: show.title })
                      }
