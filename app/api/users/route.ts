...
export async function POST(request: NextRequest) {
  ...
  if (existing.length > 0) {
    return NextResponse.json({ error: 'Korisnik s tim emailom već postoji.' }, { status: 409 });
  }

  const targetRole = role === 'admin' ? 'admin' : 'viewer';
  const LIMITS = { admin: 2, viewer: 4 };

  const [{ count }] = await sql`SELECT COUNT(*)::int AS count FROM users WHERE role = ${targetRole}` as [{ count: number }];
  if (count >= LIMITS[targetRole]) {
    const label = targetRole === 'admin' ? 'administratora' : 'preglednika';
